                                       ...
                                       cJSON *json)
{
    ...
    // cjwt->header.kid = kid->valuestring;
...
void cjwt_destroy(cjwt_t *jwt)
{
    if (jwt) {
        if(jwt->header.kid) {
            cjwt_header_destroy(&(jwt->header));
        }
...
/**
 * Cleans up header object from cjwt_get_header()
 */
void cjwt_header_destroy(cjwt_header_t* jwt_header) {
    if (jwt_header) {
        if (jwt_header->kid) {
            free(jwt_header->kid);
        }
...
/**
 * Returns header of given jwt with no validation
 */
cjwt_code_t cjwt_get_header(const char *encoded, size_t enc_len, uint32_t options, cjwt_header_t **jwt)
{
    cjwt_code_t rv = CJWTE_OK;
    struct split_jwt sections;
    const struct section *header  = NULL;
